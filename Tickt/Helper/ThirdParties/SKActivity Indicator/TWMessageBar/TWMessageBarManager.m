...
- (TWMessageBarViewController *)messageBarViewController
{
    if (!self.messageWindow)
    {
        ...
        self.messageWindow.frame = [[AppConstant getKeyWindow] frame];
...
+ (void)initialize
{
	...
	{
        // Fonts
        kTWMessageViewTitleFont = [UIFont fontWithName:@"Montserrat-SemiBold" size:16.0];
        kTWMessageViewDescriptionFont = [UIFont fontWithName:@"Muli" size:14.0];
...
- (CGRect)statusBarFrame
{
    CGRect windowFrame = NSFoundationVersionNumber <= NSFoundationVersionNumber_iOS_7_1 ? [self orientFrame:AppConstant.getKeyWindow.frame] : AppConstant.getKeyWindow.frame;
//    CGRect statusFrame = NSFoundationVersionNumber <= NSFoundationVersionNumber_iOS_7_1 ?  [self orientFrame:[UIApplication sharedApplication].statusBarFrame] : [UIApplication sharedApplication].statusBarFrame;
    if (@available(iOS 13.0, *)) {
        CGRect statusFrame = NSFoundationVersionNumber <= NSFoundationVersionNumber_iOS_7_1 ?  [self orientFrame:[AppConstant getKeyWindow].windowScene.statusBarManager.statusBarFrame] : [AppConstant getKeyWindow].windowScene.statusBarManager.statusBarFrame;
        return CGRectMake(windowFrame.origin.x, windowFrame.origin.y, windowFrame.size.width, statusFrame.size.height);
    } else {
        CGRect statusFrame = CGRectMake(0, 0, 0, 0);
        if (@available(ios 13.0, *)) {
            statusFrame = [[[[AppConstant getKeyWindow] windowScene] statusBarManager] statusBarFrame];
        } else {
            statusFrame = NSFoundationVersionNumber <= NSFoundationVersionNumber_iOS_7_1 ?  [self orientFrame:[AppConstant getKeyWindow].windowScene.statusBarManager.statusBarFrame] : [AppConstant getKeyWindow].windowScene.statusBarManager.statusBarFrame;
        }
        return CGRectMake(windowFrame.origin.x, windowFrame.origin.y, windowFrame.size.width, statusFrame.size.height);
    }
}
...
+ (void)initialize
{
	if (self == [TWDefaultMessageBarStyleSheet class])
	{
        ...
        kTWDefaultMessageBarStyleSheetSuccessBackgroundColor = [UIColor colorWithRed:0.0f green:0.482f blue:1.0f alpha:kTWMessageBarStyleSheetMessageBarAlpha]; // green
